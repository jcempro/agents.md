...
- Deve representar apenas arquivos efetivamente existentes, manifestados e relevantes ao Repositório Final, evitando referência obsoleta, interno do construtor ou auxiliar sem efeito no destino.
- Cada entrada DEVE declarar perfil `consumer-*`, origem, destino, propriedade e condição; `builder-internal` ou arquivo de `src` sem perfil bloqueia geração.
...
- Não copiar recursivamente `src/` nem inferir elegibilidade por extensão. O mesmo manifesto/perfil DEVE alimentar indexador, build, package, release, ZIP/tarball, mapa, update e publicação material deste produto.
- `verify` DEVE rejeitar microconceito citado sem definição, definição duplicada e referência Markdown explícita inexistente, fora da fonte consumidora ou com caixa divergente; arquivo real de `src` sem perfil; entrada perfilada ausente; destino duplicado; conteúdo `builder-internal`; e qualquer diferença entre manifesto, mapa, filesystem e pacote. Depois DEVE comparar cada fonte indexada com seu path materializado/transformação declarada em `./dist/`. NÃO DEVE exigir igualdade entre fonte do produto e governança ativa `./AGENTS.md`/`./.ia.rules/`, pois esses domínios possuem autoridade distinta.
- Configuração física, metadado de trabalho, comando, teste, estudo, avaliação, cenário, runtime ou documentação exclusiva do construtor DEVE permanecer fora de `src` e do pacote; conteúdo misto é separado antes da materialização.

...
> Target Construtivo distribuível: `./src/`.
>
> Topologia obrigatória: raiz do repositório `./`; estrutura-fonte exclusivamente distribuível `./src/`; raiz do artefato publicado `./dist/`. `./` organiza a governança ativa e o ambiente interno do construtor, `./src/` contém somente fontes que produzem artefato útil aos Repositórios Finais e `./dist/` é a superfície materializada distribuível; a raiz da aplicação é declarada pelo cenário e, em Web Page Like, coincide com `./dist/` e com `/` público. Esses domínios NÃO são equivalentes. [b528cf3]
...
Seu objetivo não é reproduzir texto literal, mas preservar integralmente comportamento normativo, domínios, precedências, contratos, exceções, rastreabilidade e extensibilidade. Qualquer implementação conforme este RCF deve ser semanticamente equivalente ou superior, nunca mais fraca. [c57042e]
...
**referência**: a normatização correspondente deve ser feita, mas a criação do arquivo não. [e863b8d]

### 0.0.2 Atualização declarativa, compatibilidade e limpeza

O produto-fonte destinado aos Repositórios Finais reside exclusivamente em `./src/`; reciprocamente, todo arquivo sob `./src/` DEVE possuir finalidade direta, necessária e manifestada em ao menos um perfil de distribuição ao consumidor. Dentro dele, `./src/.ia.rules/` é a única árvore estrutural publicável. A montagem usa seleção positiva pelo manifesto/perfil: copia apenas arquivos-raiz explicitamente allowlisted e materializa sob `./dist/.ia.rules/` somente contratos, configuração, runtime, cenários, recursos e fontes que o consumidor efetivamente usa. Varredura de `src/` serve a inventário e diagnóstico, nunca a autorizar cópia automática. `./dist/` e o ZIP NÃO PODEM conter outro diretório, árvore predecessora, teste, infraestrutura, configuração deste repositório canônico, estudo, avaliação, experimento, histórico, decisão temporária, protótipo, arquivo morto, código exclusivo de desenvolvimento ou referência ao layout interno. Fonte TypeScript distribuível declarada por §17 é contrato público versionado, não código exclusivo de desenvolvimento; DEVE permanecer manifestada e vinculada ao transpilado. O `package.json` distribuído deve reescrever comandos gerenciados para `.ia.rules/`; `release.json`, manifesto e validação devem rejeitar entrada não elegível, não perfilada ou fora da allowlist. Governança ativa, documentação, configuração, avaliação, runtime e infraestrutura exclusivos deste repositório canônico e sem função reutilizável permanecem fora de `src/` e do payload; regra/mecanismo do papel Construtor continua elegível por cenário técnico. [b528cf3]

O pacote recebido de release estável validado ou da branch primária é a única autoridade para o conjunto gerenciado após uma atualização. O atualizador deve ler seu manifesto versionado antes de tocar o destino e construir o resultado exclusivamente a partir das entradas, caminhos, versões e hashes recebidos. Antes do staging, o atualizador deve preparar `.gitignore`, `package.json` e manifestos análogos quando indispensáveis para versionar a árvore gerenciada recebida; essa edição é parte do update, mas deve ser mínima, rastreável, delimitada por bloco quando textual, preservadora de regras locais, e limitada a material reutilizável aplicável ao consumidor. Hash de conteúdo textual deve usar UTF-8 com LF canônico, para que checkout CRLF e ZIP/main LF sejam semanticamente iguais sem ocultar alteração material. Após download/extracao únicos, o bootstrap instalado valida o runtime mínimo manifestado e passa bastão ao atualizador contido na release. Estado autenticado `agents-update-handoff/v1` fixa a fase posterior ao download, argumentos, source, hashes, releaseRoot-fonte e targetRoot-destino; o sucessor retoma sem rede ou repetição de fase, carrega código/dependências somente da release e aplica exclusivamente no target. HMAC efêmero, paths reais, entrypoint/hashes revalidados, arquivo restrito, espera do filho e limpeza posterior impedem troca de contexto. Falha fecha sem fallback ao atualizador antigo. Conteúdo preexistente não é uma segunda fonte: serve somente para reconhecer gestão anterior, converter formato, gerar backup local ignorado de divergência, planejar remoção de legado ou restaurar uma transação interrompida. Todo path recebido deve convergir mesmo sem lock ou com edição local; arquivo que existia localmente, mas não integra o manifesto recebido, não pode sobreviver por inércia como parte da governança gerenciada. [96b3804]

Preservação não significa manter tudo. `agents.local.md`, `.ia.rules/local/`, `.ia.rules/hooks/` e adaptadores formalmente declarados são extensões do consumidor e ficam fora da descoberta, lock, plano, limpeza e sobrescrita. Colisão, edição manual ou autoria ambígua em path declarado pelo manifesto não interrompe a atualização: o conteúdo divergente é reunido em ZIP nível máximo sob `./agents-governance-backups/YYYY-MM-DD/`, com instante, identidade e versão no nome, excluído de lock/commit, informado ao usuário para exclusão e substituído. A raiz é ignorada pelo Git. Atualizador e backup devem ser cross-platform e usar APIs portáveis do runtime, sem shell, separador, comando ou path exclusivo de sistema operacional. Arquivo fora do manifesto permanece intocado; só pode ser removido quando sua gestão anterior for comprovada por lock, marcador de migração ou catálogo legado fechado. Origem inválida, path inseguro ou falha de integridade bloqueiam antes de qualquer escrita. A troca deve preparar, validar e restaurar integralmente em falha; o lock novo é gravado por último. [f19fc09]

Autoridade de script compartilhado é determinada por proveniência local verificável, finalidade e namespace, nunca pelo nome isolado nem pela inclusão unilateral em `agentsGovernance` remoto. Em Repositório Final, script preexistente sem prefixo reservado — inclusive `build`, `check`, `clean`, `dev-live`, `lint`, `prepare`, `publish`, `release` e `test` — pertence ao produto até declaração local explícita em contrário e DEVE permanecer byte a byte; o manifesto remoto PODE instalar default apenas quando o nome estiver ausente, registrar que a governança criou aquele valor e, em atualização posterior, substituir somente valor ainda idêntico à última projeção instalada. `agent:*`, `agents:*` e `shared:*` permanecem namespaces gerenciados; `update:agents` e aliases oficiais declarados permanecem entradas gerenciadas obrigatórias, mas composição com um script de produto só ocorre por hook/adaptador formal e nunca por sobrescrita. Alteração local posterior de default instalado revoga a substituição automática, preserva o valor e exige diagnóstico/composição explícita. [05f2ed9]

O papel do repositório que executa um gate DEVE ser explícito e autenticável por configuração/proveniência local, não inferido por existência de `src`, nome do pacote, path do runtime ou metadado distribuído comum a todos. No construtor canônico desta Norma, `agent:verify` valida fonte positiva, TypeScript, grafo, RCF, `dist`, mapa, ZIP e demais contratos do produto normativo; em Repositório Final consumidor, valida somente integridade da governança instalada, manifesto, hashes, configuração, extensões e compatibilidade aplicáveis, e compõe/delega o gate do produto por comando ou hook declarado sem presumir sucesso. Ausência de gate do produto é resultado separado e acionável, não autorização para executar o pipeline do construtor nem sucesso implícito. [05f2ed9]

Preservação de extensão abrange a identidade autorizada em qualquer layout histórico suportável: `agents.local.md` na raiz ou sob a árvore operacional vigente/predecessora, diretórios `local/` e `hooks/` vigentes ou predecessores e adaptadores apontados por configuração, mapa, lock, marcador ou contrato anterior. Mudança de raiz, nome ou estrutura NÃO autoriza ignorar nem apagar; o atualizador primeiro descobre e classifica, depois conserva no local ainda válido ou migra ao destino canônico com conteúdo integral, hash, path de origem, marcador de herança e colisão não destrutiva. Estrutura desconhecida mas potencialmente legítima permanece isolada e preservada até classificação; limpeza só alcança autoria gerenciada positivamente comprovada. [f7b6a24]

Toda mudança de formato, caminho, notação, nome, recurso ou redução de estrutura deve declarar uma versão de linguagem/formato, marcador de variação e conversor da versão imediatamente anterior. Conversores são patrimônio de compatibilidade: permanecem versionados no histórico e distribuídos enquanto a origem puder encontrar estado anterior; a retirada do arquivo antigo exige equivalência ou pseudo-equivalência transitória explicitamente marcada. Um parser comum deve interpretar as versões declaradas e delegar apenas a conversão correspondente, evitando leitores paralelos e evolução estrutural implícita. Arquivos de configuração equivalentes DEVERIAM adotar o mesmo padrão, com descritor formal e parser único, pois a recomendação reduz acoplamento e permite limpeza segura sem transformar convenção local em obrigação absoluta. [ce7f1e0]

### 0.0.3 Identidade operacional, construtor e consumidor

O **repositório de execução** é somente o repositório fisicamente aberto. Antes de decidir, a IA construtora ou consumidora DEVE verificar sua função; abertura física NÃO prova ser construtor, consumidor, upstream, origem canônica ou destino de issue. Neste produto, a evidência inicial classifica o repositório aberto como **construtor**, salvo declaração autoritativa contrária. [5443d42]
...
### 0.0.14 Elegibilidade da fonte e segregação interna do construtor

`./src/` é fronteira semântica, não mero diretório de trabalho: contém exclusivamente fonte destinada à construção de artefato útil a Repositório Final. Um arquivo só é elegível quando, cumulativamente: **(1)** possui consumidor final e finalidade operacional, normativa, configuracional ou de bootstrap identificáveis; **(2)** produz efeito necessário em perfil de distribuição declarado; **(3)** possui origem, destino, propriedade, obrigatoriedade/condição, política de atualização e validação no manifesto; **(4)** não é estado, evidência, estudo, avaliação, experimento, histórico, decisão temporária, protótipo, teste, infraestrutura ou apoio exclusivo do construtor; e **(5)** sua presença no destino oferece benefício material superior ao custo, superfície e risco. Arquivo sem perfil, consumidor ou efeito direto é não conforme em `src/`, mesmo quando a allowlist atualmente o omita.

Perfis fechados desta arquitetura são: `consumer-core` para Norma e contratos transversais usados no destino; `consumer-runtime` para código executável necessário ao destino; `consumer-scenario` para papel, cenário técnico/produto ou capacidade condicional instalável, inclusive o papel Construtor; `consumer-bootstrap` para inicialização expressamente publicável; e `generated-release` para pacote, manifesto, mapa, índice e metadado produzidos no build. `builder-internal` classifica somente material do desenvolvimento deste repositório canônico sem valor reutilizável para uma instalação da Norma e é proibido em `src`, `dist`, pacote, ZIP, release, mapa recebido ou atualização do consumidor. Aplicabilidade por papel (`common`, `final`, `constructor`, `dual`) e elegibilidade de distribuição são eixos independentes: regra exclusiva do papel Construtor PODE e DEVE ser distribuída como cenário técnico quando integra o produto reutilizável. Entrada opcional continua manifestada e é carregada/materializada conforme contrato; “potencialmente útil”, proximidade com arquivo público ou extensão permitida não satisfazem elegibilidade. [b528cf3]

A raiz `./` é contêiner arquitetural do repositório, não área proibida. Além de governança ativa, documentação, manifesto, automação e infraestrutura transversal, PODE conter diretamente arquivo ou diretório interno do construtor quando responsabilidade, descoberta, convenção da ferramenta e coesão o justifiquem. `./constructor/` é o namespace canônico para coleções exclusivas sem raiz convencional superior, subdividido por finalidade como `docs/`, `evaluations/`, `experiments/`, `history/` e `runtime/`; `./config/`, `./test/`, `./.github/` e equivalentes de ecossistema permanecem na raiz quando sua convenção/integração assim exigir. Arquivo interno isolado PODE permanecer diretamente em `./` quando isso for mais claro que criar hierarquia artificial. Localização não concede autoridade normativa nem elegibilidade de release. [b528cf3]

`./AGENTS.md` e `./.ia.rules/` da raiz conservam integralmente suas autoridades, proteções, contratos de atualização e namespaces próprios. `./AGENTS.md` rege a execução deste construtor e nunca é convertido em fonte de payload por estar na raiz; a fonte `src/AGENTS.md` continua cobrindo Final e Construtor por roteamento técnico. `./.ia.rules/` ativo contém governança instalada, estado e extensões autorizadas; não é depósito genérico de estudo, experimento ou documentação interna. Fonte de runtime exclusiva deste repositório canônico, sem capacidade reutilizável do papel Construtor, PODE residir em `./constructor/runtime/` e gerar projeção sob `./.ia.rules/` por mecanismo manifestado, sem tornar a projeção fonte nem distribuí-la ao consumidor. As restrições desses dois artefatos NÃO proíbem outros conteúdos internos na raiz ou em namespace arquitetural adequado. [b528cf3]

CONTRADIÇÃO DETECTADA: `AGENTS.md` §§0.13–0.14 ainda direcionam implementação/recurso interno a `src`/`.ia.rules` e restringem a raiz, enquanto a solicitação humana exige `src` exclusivamente distribuível e permite internos organizados na raiz — Aplicando evolução faseada: esta FT fixa o contrato no RCF; FT-057 DEVE atualizar a Norma Operacional ativa e distribuível antes de qualquer movimentação; até essa conclusão, a precedência do `AGENTS.md` vigente impede a FT-058, sem invalidar o requisito novo. [b528cf3]

Excluir do release é necessário, mas insuficiente. Ao classificar entrada como `builder-internal`, a mesma FT DEVE reavaliar sua localização, referências, dono, persistência e destino; movê-la para fora de `src`, consolidá-la em autoridade existente ou removê-la quando morta, sempre com histórico Git e sem duplicação autoritativa. Conteúdo misto DEVE ser separado antes da exclusão: parte útil ao consumidor permanece em `src` e parte exclusiva do construtor vai ao ambiente interno. Arquivo inteiro NÃO DEVE ser mantido no payload para conservar poucos trechos úteis nem removido se ainda contiver contrato consumidor; split proporcional precede ambos. [b528cf3]
...
`AGENTS.md` DEVE funcionar nos papéis Final e Construtor e roteá-los como contextos cumulativos; norma do Construtor não é removida, apenas segregada em papel/cenário técnico para evitar carregamento indevido no Final comum. `resources/workflows.md`, metacontratos de build/release e demais módulos com vocabulário “construir” NÃO são internos por nome: permanecem elegíveis quando regem operação que um Repositório Final executa sobre o próprio produto, uma instalação que exerce papel Construtor ou recurso reutilizável. Classificação considera finalidade e efeito no destino, nunca diretório, título ou papel nominal isolado. A FT-058 DEVE inventariar cada arquivo e registrar a decisão; achado adicional aplica os mesmos critérios. [b528cf3]

Build, `release`, empacotamento e qualquer `publish` que materialize artefato deste produto DEVEM consumir a mesma seleção positiva versionada. Índice de fontes distribuíveis enumera somente `src` elegível; inventário interno deste repositório é separado, não é incorporado ao índice consumidor e não concede publicação. `release.json`, mapa, pacote e ZIP derivam do mesmo conjunto lógico e rejeitam diferença. Configuração física deste repositório, teste interno, workflow interno, RCF/TODO/memória, `constructor/`, `state/requests`, `state/contexts`, Registro Decisório local e qualquer path fora dos perfis consumidores são exclusões obrigatórias. Comando de inbox e mecanismo do papel Construtor permanecem quando perfilados como cenário técnico reutilizável; somente variante exclusivamente local é excluída. Artefato gerado só ingressa pelo perfil `generated-release`, com origem determinística e sem servir de segunda fonte. [b528cf3]

Reorganização DEVE preservar comportamento e contratos públicos dos Repositórios Finais, scripts, workflows, referências, documentação, atualização e compatibilidade, inclusive o papel Construtor. A única alteração externa automática autorizada é a retirada de material que nunca deveria compor o produto consumidor. Comando, nó ou arquivo `builder-internal`, exclusivo deste repositório canônico, não constitui API consumidor por ter vazado em release anterior; sua remoção DEVE ser declarada no mapa como legado `builder-internal-leak`, com migração/limpeza segura. Funcionalidade compartilhada e cenário técnico Construtor permanecem. Toda movimentação atualiza links, imports, requires, índices, manifests, hashes, configuração, testes e documentação no mesmo ciclo; alias permanente, cópia residual e referência ao path antigo são proibidos, salvo conversor transitório externamente justificado. [b528cf3]

Validação possui dois sentidos. **Completude de `src`:** cada arquivo real tem perfil consumidor, destino, dono, efeito e referência; diretório vazio, arquivo morto, interno ou não manifestado falha. **Pureza do payload:** cada entrada de `dist`, pacote, ZIP, release e publicação deriva de fonte elegível ou geração declarada e possui utilidade no Repositório Final; excedente, omissão, configuração do construtor, referência quebrada ou conteúdo interno falha. A suíte DEVE injetar sentinelas de avaliação/experimento/configuração interna, provar rejeição quando sob `src` e ausência quando fora; gerar artefato real; comparar manifesto↔mapa↔filesystem↔ZIP/tarball; validar imports/links/workflows/scripts/índices; e confirmar que o delta público se limita às exclusões autorizadas e metadados/hashes derivados. Verificação por nome/padrão é defesa adicional, nunca substitui perfil e análise de efeito. [b528cf3]

### 0.0.15 Fronteira absoluta do repositório associado à solicitação

Cada prompt, tarefa, sessão operacional ou equivalente possui exatamente um repositório corrente: a raiz Git inequivocamente associada à solicitação e governada pelo `AGENTS.md` aplicável nessa raiz. Somente arquivos subordinados a essa autoridade e fisicamente pertencentes ao mesmo repositório PODEM ser criados, editados, movidos, renomeados, excluídos, formatados, regenerados, versionados ou publicados. IDE/workspace com múltiplas roots, relação funcional, dependência, importação, consumo, referência, build conjunto, monorepo lógico, documentação cruzada, mesma autoria/organização ou necessidade de corrigir a origem NÃO ampliam autorização. Edição cruzada é violação crítica de governança. [05f2ed9]

Antes de qualquer escrita, efeito destrutivo ou Git mutável, o mecanismo DEVE determinar a raiz autorizada, o `AGENTS.md` aplicável, os limites físicos e todas as fronteiras internas com identidade própria. Submódulo, worktree, repositório aninhado, subrepositório, diretório Git independente, dependência vendorizada/externa, `node_modules`, cache, terceiro e artefato gerado não editável permanecem excluídos mesmo sob a árvore textual da raiz. Operação Git iniciada pela IA DEVE usar exclusivamente a raiz corrente e NÃO PODE recorrer, percorrer ou delegar a outra identidade Git. [05f2ed9]

Autorização usa path absoluto canônico e identidade física do destino. Comparação por prefixo textual é insuficiente: `..`, symlink, junction, mount, alias, hardlink quando detectável, path com caixa/normalização divergente ou qualquer indireção que resolva fora da raiz ou através de fronteira excluída DEVE ser recusada antes do primeiro efeito, em Windows e demais plataformas suportadas. Validação segue cada componente existente pelo path real e, para destino novo, valida o ancestral real mais próximo e revalida depois da criação antes da substituição atômica. [05f2ed9]

Recurso externo PODE ser lido somente quando necessário a diagnóstico, contrato ou integração e quando a leitura for permitida; leitura nunca concede escrita. Se a solução integral exigir alteração em outro repositório, a execução conclui apenas o que é autorizado no corrente, registra a dependência externa e exige nova solicitação explicitamente associada ao outro repositório. Norma do repositório corrente NÃO PODE ser projetada sobre o externo, e indisponibilidade de autoridade externa não justifica contorno por link, script global, submódulo ou operação recursiva. [05f2ed9]

A salvaguarda programática comum DEVE anteceder toda API oficial de escrita, exclusão, movimentação, geração, staging, commit, branch, merge, reset, clean, publish e release, receber raiz e destino explícitos e falhar de forma íntegra sem efeito parcial. A matriz de aceite inclui roots irmãs no mesmo workspace, biblioteca externa, submódulo, repositório aninhado, worktree, symlink/junction interno para fora, traversal, múltiplas roots, `node_modules`, dependência `file:`/link, arquivo externo referenciado pelo código, Git externo e arquivo interno legítimo; em todos os externos, bytes e estado Git permanecem idênticos, enquanto o interno autorizado continua editável. [05f2ed9]

### 0.0 Topologia, segregação e precedência arquitetural

A raiz do repositório contém governança ativa e o ambiente interno organizado do construtor conforme §0.0.14; raiz não significa artefato publicado. Implementação destinada ao consumidor permanece em `./src/`; implementação exclusiva do construtor permanece fora dela, em raiz convencional ou `./constructor/`. Essa estrutura-fonte NÃO é a raiz da aplicação. Exceção de localização exige responsabilidade arquitetural, imposição verificável de framework/ecossistema/gerador/compilador ou ganho de organização demonstrável; conveniência isolada não basta. `./dist/` contém somente artefato consumidor selecionado e NÃO DEVE depender de `./src/` ou `./constructor/` em caminho, manifesto ou runtime. Em Web Page Like, raiz da aplicação e `./dist/`/saída declarada correspondem ao `/` público percebido e NÃO expõem fonte interna. AGENTS possui precedência operacional: RCF especializa o produto sem deslocar governança ativa ou reduzir segregação. Neste produtor, `./src/AGENTS.md` e associados elegíveis são a fonte construtiva da governança distribuível; `./AGENTS.md`, associados ativos e `./constructor/` não são payload. Após materialização no consumidor, fonte distribuível torna-se núcleo gerenciado e especialização usa somente `agents.local.md`, `./.ia.rules/local/`, `./.ia.rules/hooks/` ou adaptador subordinado/preservável. Espelho entre ativo e fonte valida o pacote, não transforma interno em distribuível nem participa do `agents:update` alheio. [b528cf3]

Excepcionalmente no construtor, quando defeito cíclico tecnicamente justificado impedir o caminho canônico de corrigir, construir ou materializar o produto, solicitação humana explícita PODE autorizar correção mínima no núcleo gerenciado ativo. A FT DEVE registrar bloqueio, justificativa, arquivos e limite; aplicar a autoridade definitiva em `./src/`, sincronizar implementação/contrato, regenerar derivados e comprovar validação antes do fechamento. A exceção NÃO alcança consumidor, não autoriza especialização local e não decorre de conveniência, urgência ou mera dificuldade. [e413300]

### 0.0.5 Raiz operacional canônica e rastreabilidade interna

`./.ia.rules/` é oficialmente a raiz interna canônica e SSOT operacional do ecossistema governado por AGENTS, encerrando a árvore predecessora como destino ativo. Deve concentrar regras auxiliares, contratos, configurações, scripts, templates, manifestos, mapas, metadados, cenários, automações e artefatos auxiliares. Novo projeto e novo conteúdo usam exclusivamente essa raiz; arquivo equivalente disperso é não conforme. `./.ia.rules/` permanece reconhecível somente como legado de entrada durante migração versionada, nunca como destino de nova regra ou capacidade. [da480fc]

Cada release mantém sob `./.ia.rules/` uma versão atual explícita, um manifesto versionado exaustivo com arquivos, tipos, destinos, hashes e perfil de distribuição e uma projeção sanitizada e rastreável do `package.json`. O manifesto é a autoridade para auditoria, comparação, atualização e limpeza; varredura apenas diagnostica ou converte legado. O mapa local de hooks, extensões e adaptadores é separado do manifesto gerenciado, declara namespaces preserváveis e deve ser atualizado pelo repositório consumidor. [823ef85]

Cada release também mantém `./.ia.rules/distribution/distribution-map-<versao>.json`, referenciado por `release.json`, com classificação de arquivos efetivos, potenciais, obrigatórios, opcionais, condicionais, gerados e obsoletos; cada entrada declara path seguro, tipo, destino, hash/tamanho quando aplicável, propriedade e políticas de preservação, atualização e remoção. Ausência, JSON inválido ou semântica inválida no mapa da release recebida bloqueiam antes de escrita. Ausência ou corrupção do mapa local anterior não bloqueia convergência para release válida: o atualizador registra diagnóstico, ignora o mapa antigo e executa transição inicial conservadora, preservando extensões e autoria indeterminada. [da480fc]

Limpeza compara o estado real ao manifesto vigente e só remove path cuja gestão ou condição de legado esteja comprovada. Deve ser determinística, hookável, auditável, transacional e reversível em falha; autoria ou gestão indeterminada bloqueia remoção. Ambiente construtor pode conter ferramentas internas, enquanto o pacote consumidor conserva somente contratos públicos, configuração mínima, runtime necessário, scripts reutilizáveis e respectivas fontes distribuíveis exigidas por §17. Seu `package.json` exclui desenvolvimento exclusivo do construtor, experimento, automação interna, dependência de processo interno e metadado irrelevante. [c143be5]

A migração da árvore predecessora para `./.ia.rules/` exige descritor de formato, marcador, conversor permanente da versão anterior, backup reversível, atualização de referências e validação integral antes da ativação atômica. Durante janela declarada, ferramentas podem ler ambas; escrita nova e estado final usam somente `./.ia.rules/`. Após conversão válida, coexistência ativa é proibida e o legado gerenciado é limpo conforme manifesto. Aceite comprova estrutura, versão, manifesto, hashes, pacote sanitizado, preservação das extensões, limpeza simulada e ausência de arquivo gerenciado excedente ou divergente. [da480fc]
...
- nenhuma regra pode ser omitida por parecer secundária. [c57042e]
...
Pertencem ao `AGENTS.md` global regras que governem exclusivamente como a IA deve ler, raciocinar, planejar, registrar, alterar, validar, versionar, retomar e entregar trabalho, inclusive: [c57042e]
...
Essas regras podem e devem ser explícitas, concretas e determinísticas, pois não constituem regras de negócio do projeto/reposiório. [e4124e9]

**IMPORTANTE:** Distinguir rigorosamente o domínio normativo de IA do repositório do domínio de negócio. `./AGENTS.md`, `./.ia.rules/` e demais importações, pertencem ao repositório enquanto projeto e constituem as únicas normas, diretrizes e metadados válidos para orientar a atuação da IA. Já `./src/AGENTS.md`, `./src/.ia.rules/` e demais importações, aninhadadas sob `src`, `dist` ou outra estrutura equivalente, pertencem ao objeto de negócio produzido ou mantido pelo repositório e, embora possam possuir estrutura, nomenclatura ou conteúdo semelhantes, devem ser tratados exclusivamente como artefatos do projeto. A IA poderá lê-los, validá-los, compará-los, gerar, editar, refatorar ou manter seu conteúdo, porém jamais deverá interpretá-los como normas aplicáveis à sua própria atuação, incorporá-los ao seu contexto normativo, alterar seu comportamento com base neles ou permitir que influenciem, substituam, complementem ou contaminem as diretrizes vigentes do domínio normativo, salvo determinação explícita em sentido contrário. [f46a94b]
...
O `AGENTS.md` pode referir-se abstratamente a esses conceitos, mas não criá-los, particularizá-los ou substituí-los. [c57042e]

### 1.3 Domínio dos cenários

Arquivos de cenário contêm regras reutilizáveis de uma categoria técnica/funcional amplamente reproduzível. Podem ser concretos quanto às convenções oficiais desse cenário — comandos, portas, ferramentas preferenciais, capacidades, UX, build ou publicação — desde que: [c57042e]
...
2. **Projeto/negócio:** RCF global → RCF específico → README → memória operacional → demais documentos; `AGENTS.md` permanece obrigatório quanto ao método. [c57042e]
...
A arquitetura deve suportar: [c57042e]
...
A localização física dos cenários pode variar por decisão normativa da IA, desde que: [c57042e]
...
Repetição só é admitida como microresumo com referência explícita à autoridade original. Referência DEVE resolver path e identificador/seção existentes; nome legado, destino implícito, path relativo ambíguo ou referência circular são não conformidade. [62a0299]

---

## 3. Contrato do `AGENTS.md` global

A reconstrução deve preservar a numeração e o propósito das seções globais. Redação pode ser otimizada; conteúdo normativo não. [c57042e]

### 3.1 Seção 0 — Finalidade, autoridade e portabilidade

Deve estabelecer: [c57042e]
...
- imutabilidade, no consumidor, do AGENTS e associado gerenciado para especialização; uso obrigatório de extensão local preservável e subordinada; [1b81f78]
- bloqueio de manutenção de terceiros, salvo incorporação definitiva ao repositório.

### 3.2 Seção 1 — Domínios e precedência

Deve normatizar integralmente §1 deste RCF e impedir: [c57042e]
...
- conclusão técnica pendente de validação do desenvolvedor permanece somente em registro mínimo e retomável;
- validação concluída externaliza integralmente o histórico, indexa-o e remove imediatamente a FT do estado corrente;
- `continue.ia` NÃO PODE crescer cumulativamente por FTs encerradas; rastreabilidade exige histórico segmentado e roteável, não retenção monolítica; [96b3804]
...
- não acumular etapas;
- tarefa mínima pode consolidar commit na etapa; [c57042e]
...
Devem preservar: [c57042e]

- regressão proibida em arquitetura, negócio, UX, API, build, cache, desempenho, CI/CD, publicação, bundles e produto final; [c57042e]
...
Configuração mutável ou personalizável DEVE ter uma única raiz `./config/`, com arquivos por contexto, schema e parser compartilhado; CLI → ambiente → configuração local → default permanece a precedência. Código só conserva constante intrínseca ao algoritmo. Paths de aplicação/artefato, URLs, portas, workflow, branches, estratégia web/offline e metadados de autoria/licença pertencem à configuração. Build e dev-live consomem essa raiz; parâmetro de CI/desenvolvimento não integra runtime final. Web usa arquivo versionado cacheável ou header global justificado; bundle offline/móvel embute apenas o subconjunto requerido ou URL homologada. [5982904]

`dev-live` DEVE ser previsível em todos os consumidores: `127.0.0.1`, porta `4000`, HTTP, sem proxy e reload por watch são defaults portáteis, substituíveis somente por configuração local declarada. Ausência de servidor aplicável exige dispensa explícita, não script ambíguo. [5982904]

Build DEVE inserir e validar banner comentável usando a mesma configuração central de autoria, upstream e licença. Minificação/compilação não autoriza removê-lo. README encerra com Autoria, Repositório e Licença equivalentes; ausência de dado permanece ausência, sem inferência. [91b7309]
...
Toda entrega deve terminar com: [c57042e]
...
DEVE normatizar API operacional local por comandos padronizados, determinísticos, não interativos e reutilizáveis, preferencialmente `agent:*`, para reduzir contexto, tokens, processamento por LLM, tempo, erro e acoplamento. Havendo comando equivalente, o agente DEVE usá-lo antes de compor comandos manuais de sistema, Git ou ecossistema. [352cdd5]

Cada comando DEVE: [352cdd5]
...
Sequência recorrente, multicomando ou consumidora de contexto DEVE virar comando único da API. A API DEVE evoluir continuamente para deslocar trabalho mecânico da LLM sem ocultar erro, decisão, valor ou rastreabilidade. [352cdd5]
...
Novos comandos PODEM ser adicionados sem enfraquecer os canônicos. Alias NÃO substitui nome padronizado. Comando inaplicável DEVE possuir dispensa expressa por autoridade normativa competente. [352cdd5]

---

## 4. Contrato do item 17 — Núcleo de cenários

O item 17 não contém regras específicas de Web Page Like. Deve conter somente: [c57042e]
...
Cenário é especialização normativa reutilizável aplicável a tipo de projeto, entrega ou contexto. A lista é aberta, cumulativa e não exaustiva. Novo cenário deve ser adicionado em arquivo independente e indexado, sem expandir estruturalmente o AGENTS. [c57042e]

### 4.2 Diretrizes gerais

Todos os cenários devem observar: [c57042e]
...
Arquivo ausente, ilegível, divergente ou com dependência irresolvida é falha normativa; inferência silenciosa é proibida. Cenário atômico sem módulos aplica o próprio `scenario.md` integralmente. [62a0299]

### 4.5 Índice inicial

O índice deve registrar: [c57042e]
...
A matriz deve gerar **42 combinações**, preservando capacidades: [c57042e]
...
Legenda obrigatória: Dev, Prod, Lib, Bundle e Offline; offline incorpora todos os assets necessários quando tecnicamente possível. [c57042e]
...
Proibidos comandos redundantes como `vite-dev`, `react-build`, `jekyll-build`, `publish-react` quando houver equivalentes semânticos. [c57042e]
...
`commit` aceita mensagem e pode coordenar add/commit/push; não ocultar falha, descartar alteração, forçar push, contornar proteção ou executar destruição sem confirmação. [c57042e]

### 5.1.5 Padronização e build

Padronizar portas, parâmetros, variáveis, targets, artefatos, diretórios de saída e comportamentos Dev/Prod/Lib/Bundle/Offline. Diferença tecnológica deve ser transparente. [c57042e]
...
Conteúdo essencial não pode depender exclusivamente de JS quando houver alternativa nativa. Priorizar TS para código novo quando compatível. [c57042e]
...
- reutilizar biblioteca existente; Font Awesome pode ser padrão se já usado; [c57042e]
- padrão escuro na ausência de preferência, salvo norma superior;
- persistência não pode quebrar renderização. [c57042e]
...
Pode usar `404.main.html` como parcial; apresentação terminal somente por identidade/RCF. [c57042e]

### 5.1.14 Compactação HTML

Somente produção; incluir gerados e copiados; remover vazios/indentação dispensável; preservar `script`, `style`, `pre`, `textarea`, `template`; manter quebras não vazias; não single-line; não minificar JS/CSS/Base64 incidentalmente; evitar dependência se hook local bastar; testes obrigatórios. [c57042e]
...
Cadência e horário DEVEM ser declarados pela política editorial, com zona de publicação explícita; horário de corte, quando adotado, DEVE converter fuso explicitamente. [b1d7b8d]
...
Sem overflow; validar na menor viewport e nos pontos de quebra declarados pela baseline do projeto. Na ausência de baseline explícita, a RCF específica DEVE registrar a superfície representativa e o critério de responsividade antes da implementação. [b1d7b8d]
...
- identidade e resumo obrigatórios declarados pelo adaptador; [b1d7b8d]
...
- subcitação pode ser itálica; [c57042e]
...
Notas de rodapé devem apontar, retornar, funcionar por teclado e mobile. Referências/Bibliografia devem ser geradas de notas/metadados quando possível; padrão bibliográfico decorre da política editorial, jurisdição ou RCF específico. [b1d7b8d]
...
- regra global não pode absorver particularidade de cenário; [c57042e]
- cenário não pode absorver particularidade de projeto; [c57042e]
...
- artefatos da própria IA podem manter nomenclatura, paths e valores determinísticos. [c57042e]
...
11. A arquitetura pode ser recriada somente com este RCF? [352cdd5]

---

## 8. Saída esperada

A implementação deste RCF deve produzir, conforme escopo: [c57042e]
...
O repositório deve possuir um arquivo indexador normativo localizado no root, denominado `index.json`. [c57042e]

Objetivo: mapear cada fonte real, útil e elegível ao consumidor em `./src/`, indicando nome, caminho relativo real, perfil de distribuição e destino a partir de `./`. Inventário interno do construtor é separado e nunca ingressa por este índice.

Requisitos:

- O arquivo deve ser gerado e mantido automaticamente sempre que aplicável. [c57042e]
- Deve utilizar formato JSON válido. [c57042e]
- Deve ser minificado para reduzir tamanho e consumo desnecessário de tokens/processamento. [c57042e]
- Deve representar apenas arquivos efetivamente existentes, manifestados e relevantes ao Repositório Final, evitando referência obsoleta, interno do construtor ou auxiliar sem efeito no destino. [b528cf3]
- Cada entrada DEVE declarar perfil `consumer-*`, origem, destino, propriedade e condição; `builder-internal` ou arquivo de `src` sem perfil bloqueia geração. [b528cf3]
- A descoberta de arquivo por extensão ou varredura NÃO autoriza distribuição: ela somente detecta excedente, omissão e divergência contra o manifesto positivo.

## 10. Dist

Deve existir comando NPM ou mecanismo equivalente capaz de gerar releases otimizados no diretório `./dist/`. [c57042e]

A geração deve: [c57042e]

- Produzir a saída final destinada ao Repositório Final exclusivamente a partir das entradas `consumer-*` de `./src/` e dos artefatos `generated-release` declarados.
...
- Incluir `package.json` executável com `agentsGovernance` que declare scripts e dependências gerenciados; na atualização de um anfitrião, esse manifesto DEVE ser mesclado sem substituir os campos, scripts, dependências, locks ou configurações não declarados. [00f77ba]
- Incluir manifesto de atualização com linguagem e versão declaradas, conjunto exaustivo de arquivos gerenciados e marcadores de variação; atualização de release ou branch deve usar esse manifesto como fonte exclusiva, usando lock anterior apenas para conversão e limpeza comprovada. [ce7f1e0]
- Não copiar recursivamente `src/` nem inferir elegibilidade por extensão. O mesmo manifesto/perfil DEVE alimentar indexador, build, package, release, ZIP/tarball, mapa, update e publicação material deste produto. [b528cf3]
- `verify` DEVE rejeitar microconceito citado sem definição, definição duplicada e referência Markdown explícita inexistente, fora da fonte consumidora ou com caixa divergente; arquivo real de `src` sem perfil; entrada perfilada ausente; destino duplicado; conteúdo `builder-internal`; e qualquer diferença entre manifesto, mapa, filesystem e pacote. Depois DEVE comparar cada fonte indexada com seu path materializado/transformação declarada em `./dist/`. NÃO DEVE exigir igualdade entre fonte do produto e governança ativa `./AGENTS.md`/`./.ia.rules/`, pois esses domínios possuem autoridade distinta. [b528cf3]
- Configuração física, metadado de trabalho, comando, teste, estudo, avaliação, cenário, runtime ou documentação exclusiva do construtor DEVE permanecer fora de `src` e do pacote; conteúdo misto é separado antes da materialização. [b528cf3]

O processo deve ser resiliente, prevendo falhas de ambiente, inconsistências, erros temporários, conflitos e etapas parcialmente concluídas, utilizando validações, tratamentos adequados e retries quando aplicável. [c57042e]
...
O processo deve: [c57042e]

- Gerar o conteúdo de `./dist/`.
- Comprovar que `./dist/`, pacote e ZIP contêm exatamente a mesma seleção positiva de perfis consumidores e nenhum `builder-internal`.
...
O fluxo deve ser resiliente, prevendo falhas, inconsistências, indisponibilidade temporária de serviços e erros de execução, utilizando validações, tratamentos e retries quando aplicável. [c57042e]
...
- O arquivo deve estar localizado no root do repositório. [c57042e]
- O arquivo NÃO DEVE possuir extensão. [c8d36c1]
- A existência do arquivo deve funcionar exclusivamente como gatilho de release. [c8abd79]
- COnteúdo do arquivo indica a versão a ser usada e usa o padrão `0.0.0-beta` ou apenas `0.0.0` ou resumido `0.0`.
- Reexecução com gatilho da mesma versão deve reutilizá-lo idempotentemente. Gatilho divergente só pode ser substituído quando a tag da versão registrada comprovar publicação concluída; sem essa prova, a divergência bloqueia a nova execução. [e413300]
...
4. O novo commit deve iniciar obrigatoriamente com texto: [c57042e]

   `release:`

   O restante da mensagem é livre.

O arquivo `release` deve funcionar apenas como sinalizador transitório, nunca como conteúdo persistente do repositório. `publish`/`publicar` são reservados exclusivamente à Publicação de Conteúdo. [c8abd79]

### 11.2 Rastreabilidade de Releases

Toda publicação de release deve gerar obrigatoriamente um commit contendo prefixo: [c57042e]
...
Antes da publicação, o processo deve: [c57042e]
...
As notas geradas devem: [c57042e]
...
O mecanismo escolhido deve priorizar: [c57042e]

- funcionamento offline quando possível;
- baixo acoplamento;
- previsibilidade;
- ausência de dependências obrigatórias externas para o processo principal de release. [c57042e]
- saída em pt-br.
...
`release.json` DEVE vincular versão, tag, commit, release anterior, arquivos, asset e hash das notas. Mesmo commit/input DEVE gerar conteúdo lógico idêntico. Hooks formais `prepare`, `verify` e `published` DEVEM permitir especialização local sem alterar mecanismo comum; hook NÃO DEVE modificar versão, tag, asset ou metadado sem contrato explícito. `latest` DEVE apontar ao release mais recente e releases anteriores DEVEM manter tag histórica. [c8abd79]

## 12. Publicação de Conteúdo

Este cenário de Negócio só se aplica quando o RCF declarar conteúdo público. `publish`/`publicar` publica artigos, páginas, posts, documentação ou equivalente por comando, manifesto, sinalizador, workflow ou arquitetura superior; integra apenas build, hospedagem, CI, cache, SEO, feed, índice, agenda e distribuição já aplicáveis. Deve usar hooks formais, ser idempotente, auditável e validar disponibilidade real. [c8abd79]

É independente de Release: NÃO DEVE criar/alterar versão, tag, asset, `latest`, nota de release ou commit `release:`. Repositório sem conteúdo publicável NÃO DEVE expor comando, gatilho, workflow ou custo deste cenário. Quando materializar conteúdo deste produto, DEVE reutilizar a elegibilidade e a seleção positiva de §0.0.14; não existe canal alternativo para publicar interno do construtor. Falha antes da disponibilidade DEVE bloquear dependente; segredo, dado privado e conteúdo inelegível NÃO DEVEM ser publicados. [b528cf3]

## 13. Autoridade operacional, mecanismos oficiais e solicitações

### 13.1 Vocabulário canônico e proteção estrutural

**Norma Operacional** é `/AGENTS.md` e associados, que regem como IA, processo, leitura, edição, validação, rastreabilidade, automação, scripts, hooks, comandos e FTs operam. **RCF** é a SSOT arquitetural declarativa, determinística e verificável do que o sistema é e DEVE fazer. **Repositório Construtor** mantém, constrói, valida ou distribui a Norma Operacional; **Repositório Final** a consome. Papéis são cumulativos. Em construtor do próprio produto, a instância instalada `/AGENTS.md` e associados é CURRENT/CONSUMED e rege a execução; `./src/AGENTS.md` e associados são SOURCE/DEVELOPED, produto futuro sem autoridade corrente. Antes de aplicar regra, DEVE-se determinar arquivo, versão, papel, escopo e autoridade. Homonímia, caminho análogo, coexistência ou compatibilidade NÃO criam identidade, shadowing, herança implícita, retroatividade nem transferência de autoridade: divergência versionada PODE ser legítima; a fonte NÃO rege antes de adoção formal, e a consumida NÃO limita, remove, enfraquece nem regride a fonte. [96b3804]
...
Todo Repositório Final e todo Repositório Construtor DEVEM manter, quando houver ao menos uma entrada elegível, em `./.ia.rules/state/decisions/refused/` seu único **Registro Decisório de Recusas**, destinado exclusivamente a preservar solicitação, proposta, decisão ou caminho de implementação expressamente recusado cuja matéria recusada continue ausente e, por isso, possa ser solicitada novamente. Abrange recusa total, parcial, inicial com benefício potencial para reconsideração e parcela descartada ou condicionada de trabalho ainda ativo. O registro e seu diretório são estado operacional local do próprio repositório: NÃO integram `src/`, dist, pacote, release, manifesto distribuível, mapa de distribuição ou atualização recebida; NÃO são copiados entre repositórios nem sincronizados com o upstream. A Norma distribuível define o contrato, mas não transporta decisões contextuais. O registro complementa, sem substituir, RCF, FT, issue, `continue.ia`, Git ou fonte da solicitação. [7d6baa5]

Inclusão exige cumulativamente: recusa expressa e fundamentada; ausência atual da matéria recusada; risco razoável de reapresentação por prompt, issue, TODO ou equivalente; evidência suficiente; e benefício de memória superior à burocracia. Não implementação isolada, backlog aceito, item meramente pendente, hipótese nunca decidida, sugestão trivial, alternativa incidental, capacidade já existente ou matéria implementada NÃO DEVEM ingressar. O registro NÃO é arquivo geral de decisões, inventário de requisitos, fila, backlog, changelog, RCF paralelo ou acumulador do que o produto já possui. [fd8a893]

O diretório contém `index.json` geral, schema versionado, modelo canônico e `records/` com registros individuais ou agrupados somente quando compartilhem contexto e permaneçam inequivocamente localizáveis. O índice é conciso, suficiente para triagem sem releitura do acervo e declara, quando aplicável: ID estável `DEC-AAAAMMDD-NNN`, chave semântica, título, estado, escopo, data da decisão, decisão, grau da recusa, motivo resumido, path do registro, artefatos relacionados, condição de reavaliação, última revisão, situação vigente, confirmação de ausência e propriedade. ID usa a data original, sequência única no repositório e nunca é renumerado ou reutilizado. Estados fechados são `RECUSADO`, `PARCIALMENTE_RECUSADO`, `RECUSADO_PARA_RECONSIDERACAO`, `EM_ANDAMENTO_COM_RESTRICOES`, `REABERTO`, `SUPERADO`, `SUBSTITUIDO` e `ACEITO_APOS_REAVALIACAO`; `ADIADO` é proibido por não distinguir espera, recusa ou aceite. `RECUSADO_PARA_RECONSIDERACAO` significa recusa vigente acompanhada de benefício potencial identificado e condição técnica concreta para análise futura; passagem do tempo, preferência ou oportunidade vaga não satisfazem a condição. Grau é `TOTAL`, `PARCIAL`, `CONDICIONAL` ou `NAO_APLICAVEL`. Estado, grau e situação vigente são dimensões distintas e NÃO DEVEM fazer uma parcela recusada representar a rejeição da solicitação inteira. [fd8a893]
...
Em toda nova solicitação, imediatamente após identificar/preservar a fonte e antes de decomposição substancial, FT, proposta de solução ou reanálise, a IA DEVE consultar prioritariamente o índice por chave semântica, finalidade, efeito, pressupostos, escopo, restrições e artefatos. Havendo candidato, lê o registro e determina se a matéria já foi submetida, expressamente recusada e permanece ausente. Automação PODE apontar candidatos, mas NÃO DEVE declarar equivalência material nem rejeitar proposta diferente sem avaliação contextual. Consulta histórica é obrigação de contexto, não lista negra, proibição permanente, precedente universal ou autoridade para impedir inovação. [fd8a893]

Reavaliação DEVE ponderar fundamentos e estado do projeto na decisão anterior; evolução arquitetural, normativa e funcional; fatos, requisitos ou limitações novos; custo estimado; probabilidade fundamentada de conclusão diversa; e risco de regressão ou retrabalho repetido. Mudança material inclui, conforme o caso, requisito, evidência, dependência, arquitetura, contrato, risco, custo, capacidade, ambiente ou restrição que altere premissa ou resultado; mera reformulação, passagem de tempo, insistência ou preferência não basta. Se a solicitação for materialmente equivalente, a recusa for expressa e nenhuma mudança técnica suficiente existir, a IA NÃO DEVE reanalisar nem criar FT para a mesma solução: preserva a decisão e encerra a triagem com referência ao ID e fundamento vigente. Reanálise apesar da ausência aparente de mudança exige justificativa técnica anterior e proporcional registrada. Hipótese manifestamente superada, incompatível, redundante, já resolvida por outro meio ou reproduzida sem fato novo PODE ser descartada imediatamente com a mesma referência. [fd8a893]

Enquanto a matéria recusada permanecer ausente, o registro NÃO DEVE ser removido porque a decisão mudou. Confirmação, revisão, superação, substituição, reabertura ou aceite posterior atualiza estado, última revisão, conclusão e evolução, preservando conteúdo, justificativa original, autorias/propriedade e razão da transição. `REABERTO` exige fato novo ou justificativa de proporcionalidade; `ACEITO_APOS_REAVALIACAO` exige fundamento, vínculo ao que mudou e permanece no acervo até implementação; `SUPERADO` e `SUBSTITUIDO` exigem destino ou motivo rastreável e também permanecem enquanto o pedido recusado continuar materialmente ausente. Mudança relevante em artefato relacionado aciona revisão do registro ou diagnóstico explícito de que permanece válido. [fd8a893]

Quando a matéria antes recusada for implementada e validada, ela sai do acervo ativo porque RCF, implementação, FT e commits passam a ser as autoridades adequadas. A remoção DEVE ocorrer em commit rastreável posterior à atualização final do registro, preservar no histórico Git a solicitação, recusa, revisões e razão da implementação e deixar referências suficientes nos artefatos permanentes. Implementação parcial remove somente a parcela efetivamente materializada; parte recusada ainda ausente permanece. Remoção por simples mudança de opinião, aceite sem implementação, limpeza temporal ou redução de volume é proibida. [fd8a893]

Em solicitação ainda ativa, registra-se somente a parcela recusada ou condicionada, com vínculo à FT/tarefa, parte aceita e trabalho que prossegue; o registro é atualizado quando a execução alterar pertinência. Sugestão trivial, alternativa sem valor histórico ou negativa cujo custo de registro supere o retrabalho razoavelmente evitável NÃO DEVE ingressar no acervo. A propriedade é sempre `repository-local`; update, build e release DEVEM ignorar e preservar o diretório local, enquanto validação de publicação DEVE rejeitar qualquer cópia dele no payload. Compartilhamento externo exige avaliação/sanitização pelo cenário apropriado e nunca replica o acervo. [7d6baa5]

Validação DEVE detectar ID/chave duplicado, `ADIADO`, estado/grau inválido, schema incompatível, path/link/referência quebrada, registro não indexado, entrada sem registro, campo obrigatório ausente, ausência não confirmada, matéria já implementada, recusa não expressa, reabertura sem justificativa, transição sem histórico, registro potencialmente obsoleto sem revisão após mudança relevante e qualquer presença do diretório/registro em fonte ou release. A validação é determinística quanto a estrutura e referências; pertinência, equivalência semântica, proporcionalidade e mudança material permanecem julgamento fundamentado. Automação só é implementada quando reduzir Custo Líquido, integra indexador/verificador oficial e NÃO cria comando ou fluxo paralelo. [7d6baa5]
...
Sucesso do atualizador significa finalidade material concluída: todos os destinos manifestados e transições oficiais reconciliados, projeção compartilhada mesclada, extensões preservadas/migradas, legado classificado, lock final coerente e verificação independente sem arquivo aplicável antigo ou parcial. Ausência de exceção, igualdade de `AGENTS.md`, índice, mapa, lock, timestamp ou qualquer sentinela isolada NÃO comprova atualização. Na dúvida sobre aplicabilidade, o fluxo processa idempotentemente e valida; otimização só omite trabalho cuja equivalência integral esteja provada. [f7b6a24]

Condição programaticamente recuperável NÃO PODE terminar em erro: runtime/handoff esperado ausente, manifesto local inconsistente, layout misto, migração parcial, arquivo inesperado preservável, working tree modificada, commit local não publicado, branch divergente ou falha de uma estratégia acionam descoberta alternativa, reconstrução derivável, runtime compatível disponível, checkpoint, isolamento temporário, worktree auxiliar interna, commit preservador ou outro fallback finito seguro. O fluxo registra problema corrigido, fallback, migração, adaptação preservada e impedimento externo em categorias distintas; somente rede indispensável sem cópia válida, permissão inalcançável, armazenamento indisponível/corrompido ou impossibilidade material demonstrada permitem exit não zero após esgotar alternativas. [05f2ed9]

Estados Git legítimos são dados a preservar, não bloqueios automáticos. O atualizador fotografa HEAD, branch, upstream, index, worktree, untracked aplicáveis e commits locais; isola somente o delta normativo, nunca reseta, descarta, sobrescreve ou inclui trabalho alheio, e restaura/reconcilia a visão original após commit/push conforme contrato. Push normativo exclusivo incapaz de coexistir com commits locais DEVE ser reestruturado por estratégia segura; falha de push conserva commit normativo validado e estado do usuário, com retomada idempotente. [05f2ed9]

Handoff ocorre tão cedo quanto seguro e possui cadeia de recuperação autenticada: runtime canônico manifestado → runtime canônico reconstruído de arquivos íntegros da release → entrypoint compatível declarado pelo descritor de transição → migração local determinística. Cada alternativa revalida origem, entrypoint, dependências, hashes e targetRoot antes de escrever; nenhuma executa código remoto nem retorna silenciosamente ao bootstrap incompatível. Ausência só é terminal quando release íntegra e recursos locais não contêm material suficiente para qualquer rota segura. [f7b6a24]

A matriz mínima comprova estado final, não apenas código zero: atualizado; antigo; mistura de versões/layouts; adaptação legada válida e realocada; runtime atual, ausente e legado; interrupção/parcial; árvore limpa e modificada; commits locais; divergência; repetição pós-sucesso e pós-interrupção; inesperado preservável; metadata inconsistente; fallback após falha; indisponibilidade externa real; nenhum aplicável desatualizado; nenhuma perda de dados; scripts próprios; `agents.local.md`, hooks e adaptadores em paths vigentes/predecessores. Fixtures de releases históricas DEVEM ser imutáveis ou autenticadas por hash e cobrir saltos suficientes para provar retrocompatibilidade transitiva. [96b3804]
...
Regra exclusiva de geração, compilação, validação, empacotamento, versão, release, atualização, distribuição e estrutura-fonte da Norma permanece no módulo/papel Construtor distribuível como cenário técnico condicional; `AGENTS.md` conserva sua descoberta e força para que a mesma Norma funcione em Final, Construtor ou dupla função. Somente estudo, configuração, evidência ou implementação exclusiva deste repositório canônico e sem valor ao papel reutilizável é `builder-internal` e sai de `src`. Regra que o Repositório Final necessita para construir o próprio produto pertence ao contrato comum, de recurso ou cenário correspondente e não se torna interna pelo verbo “construir”. Autoridade, imutabilidade e proibição de edição direta permanecem comuns/Final. Cada recurso oficial possui módulo próprio ou compartilhado apenas sob contrato realmente comum, contendo finalidade, gatilho, precedência, limites, parâmetros, entradas/saídas, estados, efeitos, extensões, hooks, fallbacks, validação e uso correto. Nomes, localização, configuração, retorno, códigos, logs, segurança e extensibilidade comuns ficam centralizados; especialização referencia, não redefine.

### 16.2 Índice e descoberta compulsória

Índice/manifesto global ultrassucinto e estruturado DEVE declarar papéis cumulativos, cenários de produto/técnicos, recursos oficiais, ID, finalidade, gatilhos, obrigatoriedade, path canônico, dependências mínimas, precedência, artefatos e condição de leitura. Formato é escolhido por compreensão, validação e Custo Líquido, mantendo `AGENTS.md` como entrada Markdown. Antes de criar solução, a IA consulta o índice; correspondência de finalidade torna obrigatória a leitura integral dos módulos aplicáveis. Módulo não carregado continua obrigatório quando o gatilho ocorrer; economia resulta da exclusão segura do inaplicável, nunca de opacidade. [c143be5]

Cada módulo declara quando ler/não ler, papel/cenário/recurso, problema, artefatos, contratos comuns, dependências, modalidade e composição simultânea. Roteamento carrega somente núcleo, índice, papéis ativos, cenários/recursos aplicáveis e dependências expressas. Ciclo, órfão, profundidade excessiva, sobreposição ambígua, responsabilidade incerta, duplicação semântica ou carregamento transitivo desnecessário DEVEM ser rejeitados. [c143be5]
...
Índice e grafo DEVEM representar `AGENTS.md` como raiz híbrida mínima e `.ia.rules/agents.inc.md` como folha comum de fallback, além das rotas modulares existentes; a aresta condicional não torna o auxiliar leitura global. Migração atualiza todas as referências cuja semântica era “corpo integral em AGENTS”, manifesta ambos os arquivos positivamente, instala/atualiza ambos na mesma transação e comprova equivalência por hash do corpo migrado, cobertura de modalidades/proibições/exceções/referências, cenários de rota e comportamento do construtor. Baseline anterior é 7.364 tokens no entrypoint; relatório final registra contagem nova, redução e custo das rotas sem ocultar o tamanho total preservado. [447bef3]

## 17. Arquitetura multilíngue e distribuição dual de scripts

### 17.1 Fontes, artefatos e runtimes

Todo script executado em Node.js DEVE ter TypeScript como fonte canônica; fonte `.js` manual é proibida. Legado Node.js em JavaScript DEVE ser inventariado e convertido preservando comportamento, contratos, entrada/saída, compatibilidade, efeitos, diagnósticos e integração. A fonte TypeScript DEVE transpilar para JavaScript compatível com Node.js `24+`, com alvo ECMAScript `ESNext` estabilizado aproximadamente um ano antes da versão corrente; baseline, toolchain e matriz DEVEM ser revistos periodicamente e só elevados por necessidade/benefício demonstrável, sem ruptura evitável. Fonte `builder-internal` obedece engenharia e toolchain, mas reside fora de `src` e não adquire perfil de distribuição. [b528cf3]

JavaScript consumidor em `dist` DEVE ser transpilado, validado, otimizado e minificado sem alterar comportamento, interoperabilidade ou diagnóstico indispensável. Cada release distribui simultaneamente TypeScript e JavaScript correspondentes somente dos recursos `consumer-runtime`, com identidade, versão, finalidade e caminho lógico inequívocos; artefato divergente, desatualizado, sem origem rastreável ou exclusivo do construtor bloqueia publicação. O consumidor PODE usar fonte, pronto ou ambos conforme ambiente/política. [b528cf3]

Release declara, globalmente e por recurso quando diferirem, versões mínima, recomendada, máxima/faixa suportada de Node.js, linguagem, runtime, compilador, transpilador, gerenciador e ferramenta; dependências, alvo, formato de módulo, plataformas testadas e limites; fonte→build; versões de toolchain; SO, shells, frameworks e ambientes validados. O build DEVE ser reproduzível sem obrigar o consumidor a reconstruir artefato pronto. [c143be5]
...
Cada técnica é aprovada, rejeitada ou `RECUSADO_PARA_RECONSIDERACAO` por métricas de aplicabilidade, compatibilidade, implementação/manutenção, artefato, precisão, cobertura, latência, tokens, interpretação, rastreabilidade, fragmentação, recuperação incompleta e regressão. O terceiro estado exige recusa vigente, benefício potencial e condição técnica concreta de retorno; adiamento vago é proibido. Tendência, analogia ou benefício presumido NÃO autorizam adoção. Ausência de ganho líquido implica rejeição, sem afastar índice/grafo determinísticos obrigatórios. Embeddings, banco vetorial, reranking por modelo, armazenamento ou serviço externo só PODEM entrar se superarem índice local, grafo, busca textual e metadados; devem ser opcionais, substituíveis e justificadamente open source/gratuitos/offline ou, se cloud, com manutenção, privacidade e custo resumidamente explicados ao desenvolvedor antes da decisão. [bb2bdaf]
...
Script canônico DEVE inspecionar fontes, validar/simular grafo, calcular caminhos, atualizar índice, gerar mapa visual e métricas README. DEVERIA atuar como conector de biblioteca open source mantida/confiável em vez de reimplementar tokenizer, salvo impossibilidade comprovada. Para cada alvo, usa tokenizer oficial ou equivalente bit a bit e registra modelo, tokenizer, versão, encoding e serialização. Múltiplos alvos têm resultados separados; estimativa NÃO é exata. Mudança de modelo/vocabulário/encoding/biblioteca invalida contagens. [c143be5]
...
Execução DEVE exibir progresso ultrassucinto com etapa, estado, arquivo/nó e falha. Para folha e híbrido terminal separadamente, índice, mapa, região gerenciada do README e saída CLI DEVEM usar a mesma série — um custo inteiro de tokens por rota válida — e apresentar contagem, mínimo, média aritmética, mediana, desvio padrão populacional e máximo: mediana é o centro ordenado ou a média dos dois centros; desvio é a raiz da média dos desvios quadráticos, usa `0` para série unitária e `n/a` para série vazia; média, mediana e desvio preservam precisão integral no cálculo e arredondam somente a exibição a duas casas, enquanto mínimo/máximo permanecem inteiros. Inclusão, rotas múltiplas e ponderação DEVEM ser determinísticas e documentadas; a região README conserva tokenizer, versão, data/commit e link ao mapa sem reformatar conteúdo adjacente; o mapa `.md` mostra nós, papéis, arestas, caminhos, custos e a semântica estatística; índice, mapa, tabela e cache possuem origem, versão, geração determinística e validação de obsolescência. [95bf00f]

### 18.5 Workflow e segurança

Push que altere `src/**/AGENTS.md`, normativo/roteador `.md` em qualquer profundidade ou script, tokenizer, configuração, índice/gerador capaz de mudar resultados DEVE acionar workflow específico. Ele valida grafo, recalcula, atualiza índice/mapa/README e registra alterações. Filtro evita arquivo alheio; commit automático tem marcador anti-recursão sem dispensar mudança material posterior. Concorrência, branch, push obsoleto e divergência DEVEM impedir sobrescrita, parcial ou artefato de revisão antiga. [c143be5]
...
FT normativa só conclui quando requisito, detalhe, exceção, motivação, relação e critério das origens estiverem no RCF sem duplicação desnecessária e as FTs posteriores estiverem coerentes. Registro temporário e contextos são removidos somente depois da auditoria bidirecional. FT de `src/` comprova equivalência RCF→Norma Operacional, autoridade, descoberta, rotas e métricas, sem implementar código. Para §13.6, a fase `src/` normatiza consulta, elegibilidade, estados, retenção e exclusão de release; o Repositório Construtor cria separadamente em seu estado local o diretório `state/decisions/refused`, índice, schema, modelo e somente recusas expressas historicamente comprovadas cuja matéria continue ausente. Valida manualmente estrutura, vínculos, ausência, não inundação e ausência no payload. A fase de código integra validação proporcional ao mecanismo oficial para detectar inconsistências estruturais e referências, sem automatizar equivalência semântica, mudança material ou rejeição. FT de código comprova contratos comuns, atualizador, TypeScript/distribuição dual, runtimes, workflows, grafo/tokenização/mapa, rastreabilidade, corpus, cross-platform, reprodutibilidade, privacidade, segurança e ausência de fluxo paralelo.

Para §0.0.14, FT-056 conclui somente com inventário integral do estado real, classificação por efeito e não por nome, casos mistos identificados, topologia interna, perfis, transição, compatibilidade e testes reconstruíveis no RCF, sem tocar fonte ou pipeline. FT-057, após autorização nova, atualiza `./AGENTS.md` ativo por autorização específica e projeta em `src/AGENTS.md`/associados a Norma reutilizável completa para Final e Construtor, segregando o segundo por papel/cenário técnico e carregamento condicional, sem retirar sua normatização, mover arquivo ou alterar código. FT-058, após autorização nova, reclassifica cada arquivo, move somente internos deste repositório, divide mistos quando necessário, preserva mecanismos reutilizáveis do Construtor, atualiza toda referência e implementa seleção positiva única para indexador/build/release/package/ZIP/update/publish, tratando vazamento histórico como legado removível. FT-059 gera artefatos reais e prova inventário↔perfil↔manifesto↔mapa↔filesystem↔pacote, scripts, workflows, links, documentação, índices, atualização e delta público limitado. Achado de código retorna à FT-058; falha transversal já proprietária da FT-052 não é duplicada e só bloqueia fechamento global quando impedir prova material.

A implementação de `github:jcempro/agents.md#2` exige que FT-060 conclua com configuração distribuída e gate de autorização reconstruíveis no RCF; FT-062 comprova `repository.json` como fonte positiva autenticada em build/mapa/ZIP/update, consumidor limpo sem configuração residual, cabeçalhos íntegros, edição divergente preservada e `upstream-share` autorizado capaz de publicar ou detectar duplicata sem referência ausente. Para `github:jcempro/agents.md#10`, FT-061 conclui com limite, entrypoint, auxiliar integral, condições de leitura, autoridade, precedência e migração projetados no RCF e na Norma; FT-063 comprova ≤500 tokens por tokenizer declarado, hash integral do corpo migrado, índice/grafo/manifesto/update, rotas seletivas, clientes compatíveis e ausência de perda. FT-059 só inicia depois dessas FTs e executa auditoria/release conjunto, sem absorver correção proprietária.

Para a Issue `github:jcempro/agents.md#11` e a atualização resiliente, a fase normativa só conclui quando separar inequivocamente autoridade do produto e da governança, defaults e valores preexistentes, construtor canônico e Final consumidor, integridade instalada e gate do produto, além de cobrir todas as extensões/históricos e classes recuperáveis sem enfraquecer a substituição obrigatória do núcleo realmente gerenciado. A fase técnica deve provar consumidor com `build`/`check`/`test`/`prepare` próprios, composição explícita, perfil correto de `agent:verify`, idempotência, backup ignorado/recuperável, matriz histórica, Git legítimo e nenhum arquivo aplicável antigo; resposta do mantenedor só prevalece para autoria gerenciada comprovada, nunca para coincidência nominal. [05f2ed9]
...
Para a TO-DO editorial/TTS, FT-091 DEVE equalizar e normatizar o RCF; FT-092 projeta somente a Norma Operacional; FT-093, após autorização nova, implementa capacidades independentes e hookable; FT-094 integra e valida. Nenhuma dessas fases publica release sem pedido, versão e autorização específicos. [8f5b95f]

Para a TO-DO de custo contextual, FT-095 DEVE normatizar método e decisão de unidade; FT-096 projeta somente o contrato/rota da Skill; FT-097, somente após FT-094 e autorização nova, implementa Skill, medições, experimentos e relatório sem aplicar recomendação material; FT-098 integra e valida. Recomendação de alteração cria FT posterior e NÃO autoriza a própria mudança; nenhuma fase publica release sem pedido, versão e autorização específicos. [8f5b95f]

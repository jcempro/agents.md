...
O sistema DEVE manter em `./.ia.rules/decisions/refused/` o único **Registro Decisório de Recusas**, destinado exclusivamente a preservar solicitação, proposta, decisão ou caminho de implementação expressamente recusado cuja matéria recusada continue ausente e, por isso, possa ser solicitada novamente. Abrange recusa total, parcial, inicial com benefício potencial para reconsideração e parcela descartada ou condicionada de trabalho ainda ativo. No construtor, a fonte distribuível correspondente reside em `src/.ia.rules/decisions/refused/`; build/update materializam o mesmo contrato no destino sem criar raiz, índice ou histórico concorrente. O registro complementa, mas NÃO substitui, RCF, FT, issue, `continue.ia`, Git ou fonte da solicitação.

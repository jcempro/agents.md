...
O pacote recebido de release estável validado ou da branch primária é a única autoridade para o conjunto gerenciado após uma atualização. O atualizador deve ler seu manifesto versionado antes de tocar o destino e construir o resultado exclusivamente a partir das entradas, caminhos, versões e hashes recebidos. Hash de conteúdo textual deve usar UTF-8 com LF canônico, para que checkout CRLF e ZIP/main LF sejam semanticamente iguais sem ocultar alteração material. Conteúdo preexistente não é uma segunda fonte: serve somente para reconhecer gestão anterior, converter formato, detectar edição manual, planejar remoção de legado ou restaurar uma transação interrompida. Assim, arquivo que existia localmente, mas não integra o manifesto recebido, não pode sobreviver por inércia como parte da governança gerenciada.

Preservação não significa manter tudo. `agents.local.md`, `.agents/local/`, `.agents/hooks/` e adaptadores formalmente declarados são extensões do consumidor e ficam fora da descoberta, lock, plano, limpeza e sobrescrita. Todo outro arquivo só pode ser removido quando sua gestão anterior for comprovada por lock, marcador de migração ou catálogo legado fechado; colisão com arquivo local não gerenciado, edição manual, origem ambígua ou caminho inseguro interrompe a atualização antes de qualquer escrita. A troca deve preparar, validar e restaurar integralmente em falha; o lock novo é gravado por último.
...
Neste produto, `jcempro/agents.md` é o sucessor canônico comprovado de `JeanCarloEM/agents.md`. A migração DEVE publicar no predecessor uma release-ponte com a mesma versão, ZIP e notas do sucessor; o ZIP DEVE conter metadado gerenciado que faça `agent:autoupdate` resolver o sucessor nas execuções seguintes sem escrita em `--check` ou `--dry-run`. O lock DEVE registrar o repositório efetivamente resolvido e não constante histórica.
...
Issue aprovada DEVE possuir identidade canônica `github:<owner>/<repo>#<numero>`. Script local e workflow online DEVEM localizar todas as issues abertas com `agents:approved`, baixar e sanitizar seus dados, persistir a inbox, criar exatamente uma FT por identidade em `continue.ia` e publicar essa correlação antes de qualquer efeito de desenvolvimento. Após push comprovado, DEVEM adicionar `agents:in-development`, comentar o início com a FT e registrar `issue_state=em_desenvolvimento`, idempotentemente. FT correlacionada concluída só integra uma versão quando `release=<versao>` for gravado antes da geração do artefato. Após o release ser publicado como `latest`, o workflow DEVE processar todas as FTs concluídas vinculadas àquela versão, comentar versão e FT, adicionar `agents:fixed`, fechar cada issue como concluída e registrar `issue_state=corrigida`; falha em qualquer issue DEVE bloquear o commit final e preservar retomada idempotente. Agendamento, evento de label e execução manual DEVEM reutilizar o mesmo script e nunca inferir correlação por título, texto ou ordem.

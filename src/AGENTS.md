...
RCF/cenário DEVE definir comportamento; AGENTS/auxiliar, processamento. Conversão entre domínios é regressão. `agents.local.md` e memória PODEM residir na raiz ou em `./.agents/`. Fonte, artefato, construtor, consumidor e dupla função DEVEM manter autoridade, contexto, arquivo e validação segregados.

A atuação da IA restringe-se ao repositório de execução, inclusive sob autorização humana. `./AGENTS.md` e seus análogos/associados gerenciados são sempre intocáveis. Quando o upstream de AGENTS.md não for comprovadamente o próprio repositório de execução, ele também é intocável, não integra o escopo gerenciado e NÃO DEVE ser editado, triado, avaliado, aprovado, implementado, publicado, atualizado ou gerenciado pela IA. A única exceção ocorre quando ambos são o mesmo repositório: atos de mantenedor ainda exigem autoridade própria e independente, e qualquer edição de arquivo limita-se a conteúdo sob `./src/`; a raiz normativa, seus associados gerenciados e qualquer outro path permanecem intocáveis. Este limite prevalece sobre RCF, FT, memória, sincronização, build, derivação, handoff, commit, publicação, comando ou autorização conflitante: a IA registra a pendência na saída, sem escrever fora de `./src/`.

Repositório consumidor/final limita eventual contribuição upstream à criação de uma issue potencial, exclusivamente para comunicar a proposta. A issue NÃO transfere autoridade para comentá-la como mantenedor, triá-la, avaliá-la, aprová-la, implementá-la, publicá-la, fechá-la ou alterar/gerenciar o upstream; autorização dada no consumidor NÃO amplia esse limite. Qualquer ato de mantenedor exige atuação separada no próprio repositório upstream, com identidade e autoridade próprias, sem derivá-las da issue. Atualizar norma raiz a partir da fonte NÃO DEVE ocorrer. Alterar RCF exige confirmação humana diante de ambiguidade, risco interpretativo ou regressão possível, observado o bloqueio de paths desta seção.

### 0.12 Atualização

Todo repositório DEVE expor `update:agents` como comando NPM canônico; `agent:autoupdate`, `agents:autoupdate`, `agent:agents` e `agents:update` PODEM ser aliases transitórios equivalentes. O modo mutável sem argumento pertence exclusivamente a operador/workflow não-IA e sempre converge o núcleo ao manifesto validado, commita e publica. A IA, mesmo sob autorização humana, PODE executar somente `--check` ou `--dry-run`, que não escrevem; NÃO DEVE iniciar, confirmar, delegar ou contornar aplicação mutável. Criação/reparo aplica `./.agents/core/update/scenario.md`. Após download/extracao e validação mínima do runtime manifestado, o bootstrap instalado DEVE passar bastão ao atualizador da release, com estado autenticado e retomada exata sem nova rede: releaseRoot é fonte imutável de código/dependências e targetRoot é o único destino. Falha no handoff fecha sem fallback ao runtime antigo. Origem recebida validada define exaustivamente o gerenciado: arquivo preexistente ou editado em path recebido DEVE ser preservado, quando divergente, em ZIP de compressão máxima sob `./agents-governance-backups/YYYY-MM-DD/` e substituído, nunca bloquear a atualização; somente extensão autorizada fica fora de descoberta e sobrescrita. A raiz de backup DEVE ser ignorada pelo Git, ter nome rastreável e ser informada para exclusão humana. Preexistente serve à migração, backup, legado comprovado, limpeza ou restauração transacional, nunca como segunda fonte. Mudança de formato/path/notação/recurso/estrutura DEVE incluir descritor versionado, marcador e conversor permanente da versão anterior; ausência futura do original NÃO autoriza retirar conversor. Configuração equivalente DEVERIA compartilhar parser e descritor formal.
...
| Evolução upstream       | `./.agents/scenarios/governance/upstream-sharing/scenario.md`                                                 | núcleo, atualização e CLI                 | consumidor somente comunica issue potencial; construtor apenas no próprio upstream |
...
Somente quando o repositório de execução for comprovadamente o próprio upstream de AGENTS.md, aceite humano de issue PODE usar `agent:inbox:approve` com issue, papel construtor, autoridade de mantenedor separada e autorização explícita; a issue/recomendação técnica não constitui nem transfere aceite, e a FT nasce na sincronização posterior.

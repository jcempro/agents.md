...
Repositório de execução é somente o aberto fisicamente; atuação e gerenciamento da IA NÃO DEVEM ultrapassá-lo, mesmo sob autorização humana. Função de construtor, consumidor, upstream ou dupla função exige evidência autoritativa. Apenas `./AGENTS.md` é norma raiz; homônimo sob fonte/artefato só assume função explicitamente declarada. `./AGENTS.md` e análogo/associado gerenciado são sempre intocáveis. O upstream de AGENTS.md também permanece intocável e NÃO DEVE ser gerenciado enquanto não for comprovadamente o próprio repositório de execução; nessa identidade excepcional, ato de mantenedor exige autoridade separada e edição de arquivo limita-se a `./src/`, sem alcançar raiz normativa, associado gerenciado ou outro path. Esse limite prevalece sobre RCF, FT, memória, sincronização, build, derivação, handoff, Git, publicação, comando e autorização conflitante; pendência é somente relatada. Construtor mantém fonte distribuível sem ela reger/sincronizar sua raiz; consumidor obedece à cópia materializada. Upstreams do consumidor, de AGENTS.md e de dependência são distintos; upstream de AGENTS.md resolve por configuração explícita → metadado oficial → origem declarada → sucessor → destino canônico validado → candidato configurado. Candidato, fork, mirror, homônimo, proximidade, autorização no consumidor ou issue NÃO provam nem transferem autoridade.

## MN-EXT — núcleo gerenciado e extensão local

Consumidor NÃO DEVE especializar núcleo gerenciado por edição, cópia oficial simulada, patch oculto ou reescrita pós-sincronização. Especialização usa somente `agents.local.md`, `./.agents/local/`, `./.agents/hooks/` ou adaptador declarado: é subordinada, carregada quando aplicável e excluída de descoberta, lock, remoção e sobrescrita. Path declarado pelo manifesto validado pertence ao núcleo: colisão, autoria ambígua ou customização nesse path DEVE gerar ZIP de compressão máxima em `./agents-governance-backups/YYYY-MM-DD/`, raiz ignorada pelo Git, e ser substituída sem bloquear convergência; colisão fora do manifesto permanece intocada. O backup inclui instante, identidade/versão, é informado para exclusão humana e não integra lock/commit. `--force` e autorização humana são incapazes de ampliar autoridade, converter customização em regra ou liberar AGENTS/associado/upstream. Somente a identidade comprovada entre repositório de execução e upstream de AGENTS.md PODE liberar manutenção da fonte distribuível, estritamente sob `./src/`; nenhum RCF, issue, hook ou dupla função amplia essa exceção. Repositório consumidor/final PODE apenas comunicar issue potencial ao upstream e NÃO DEVE ali triar, avaliar, aprovar, implementar, publicar, fechar ou alterar. Terceiro importado só PODE ser processado após incorporação definitiva.

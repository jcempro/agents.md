...
function resolveCanonicalContinueFile(rootDir) {
  const found = CANONICAL_FILES.map((name) => ({ name, path: path.join(rootDir, name) })).filter((entry) => fs.existsSync(entry.path) && fs.statSync(entry.path).isFile());
  if (found.length !== 1) {
...
function renderStatus(status) {
  ...
  const color = normalized === "concluído" ? "#15803d" : normalized === "em andamento" ? "#ca8a04" : "#64748b";
...
function isTechnicalScope(scope) {
  const value = String(scope || "").normalize("NFD").replace(/[\u0300-\u036f]/gu, "").toLocaleLowerCase("pt-BR");

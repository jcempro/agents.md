...
const ACCEPTED_ACTIONS = /* @__PURE__ */ new Set(["opened", "edited", "reopened", "labeled"]);
...
function normalizeIssue(repositorySource, issueSource, action, options) {
  ...
  const body = sanitizeIssueText(issueSource && issueSource.body).slice(0, 16e3);
...
function persistRecord(record, options = {}) {
  ...
  const existing = index.items.find((item2) => item2.key === record.key);
  if (existing) return { code: "INBOX_ALREADY_INDEXED", duplicate: true, file: existing.file, key: record.key, status: existing.status };
  const priorEvidence = index.items.find((item2) => item2.id === record.id && item2.evidenceHash === record.evidenceHash);
...
function normalizeLabels(labels) {
  return (Array.isArray(labels) ? labels : []).map((item) => sanitizeText(item && item.name || item)).filter((item) => /^[A-Za-z0-9:_ .-]{1,80}$/u.test(item)).sort();
}
function sanitizeLogin(value) {
  const login = String(value || "");
  return /^[A-Za-z0-9-]{1,39}$/u.test(login) ? login : "";
}
function detectLanguage(value) {
  return /\b(para|com|não|proposta|evidência|cenário|reutiliz)/iu.test(value) ? "pt-BR" : /\b(the|and|proposal|evidence|scenario|reuse)/iu.test(value) ? "en" : "pt-BR";
}
function localizedReason(reason, language) {
  return /^pt/iu.test(language) ? `Motivo: ${reason}.` : `Reason: ${reason}.`;
}
function publicIssueUrl(value, repository, number) {
  const expected = `https://github.com/${repository}/issues/${number}`;
  return String(value || "").startsWith(`https://github.com/${repository}/issues/`) ? String(value) : expected;
}
function isoDate(value) {
  const parsed = new Date(String(value || ""));
  return Number.isNaN(parsed.getTime()) ? "1970-01-01T00:00:00.000Z" : parsed.toISOString();
}
function recordFile(record, options) {
  const file = path.join(resolveInboxDir(options), "issues", `${record.issue.number}-${record.evidenceHash.slice(0, 16)}.json`);
  return fs.existsSync(file) ? relative(file) : "";
}
function resolveInboxDir(options) {
  return path.resolve(options.inboxDir || process.env.AGENTS_INBOX_DIR || DEFAULT_INBOX_DIR);
}
...
function validateRecord(record) {
  if (!record || record.schema !== 1 || !record.issue || !validRepository(record.source && record.source.repository)) throw new Error("REGISTRO_INBOX_INVALIDO");
}
function validateAssessment(assessment) {
  if (!assessment || assessment.schema !== 1 || !["rejected", "not_recommended", "recommended", "highly_recommended"].includes(assessment.grade)) throw new Error("AVALIACAO_INBOX_INVALIDA");
}
function positiveInt(value, name) {
  if (!/^\d+$/u.test(String(value)) || Number(value) < 1) throw new UsageError(`PARAMETRO_INVALIDO:${name}`);
  return Number(value);
}
function validRepository(value) {
  return /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(String(value || ""));
}
...
function help() {
  return "Uso: issue-inbox <event <evento.json>|fetch <numero>|evaluate <registro.json>|process <evento.json>|apply <avaliacao.json>|approve --issue <n>|self-test> [--role constructor|dual] [--authorize] [--dry-run] [--notify-collaborators]\n";
}
function selfTest() {

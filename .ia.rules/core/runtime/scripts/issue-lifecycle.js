...
function releaseCompletionTargets(memory, versionValue) {
  ...
  return fronts.filter((front) => front.status === "concluido" && front.release === version && front.issueState === "em_desenvolvimento").filter((front) => issueGroupCanClose(groups.get(front.issueId), version));
}
function issueGroupCanClose(group = [], version = "") {
  if (!group.length) return false;
  return group.every((front) => front.status === "concluido" && front.issueState === "em_desenvolvimento" && front.release === version && !hasPendingInternalTask(front.block));
}
...
function splitFronts(memory) {
  return String(memory).split(/(?=^FT-\d+\|)/mu).filter((block) => /^FT-\d+\|/u.test(block));
}
function field(block, name) {
  return (block.match(new RegExp(`^${name}=([^\\r\\n]*)`, "mu")) || [])[1] || "";
}
function appendFront(memory, front) {
  return `${String(memory).trimEnd()}

${front}
`;
}
function maxFt(memory) {
  return Math.max(0, ...Array.from(String(memory).matchAll(/^FT-(\d+)\|/gmu), (match) => Number(match[1])));
}
function updateIssueStates(memory, targets, state) {
  return targets.reduce((text, target) => replaceFieldInFront(text, target.ft, "issue_state", state), memory);
}
function replaceFieldInFront(memory, ft, name, value) {
  return String(memory).replace(new RegExp(`(^${ft}\\|[\\s\\S]*?^${name}=)[^\\r\\n]*(?=\\r?$|\\n)(?=[\\s\\S]*?(?:^FT-\\d+\\||$))`, "mu"), `$1${value}`);
}
function publicTarget(target) {
  return { ft: target.ft, issue: target.issue, issueId: target.issueId, repository: target.repository };
}
function normalizeFt(value) {
  const ft = String(value || "").toUpperCase();
  if (!/^FT-\d{3,}$/u.test(ft)) throw new UsageError("PARAMETRO_INVALIDO:ft");
  return ft;
}
function normalizeVersion(value) {
  const version = String(value || "").trim();
  if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u.test(version)) throw new UsageError("PARAMETRO_INVALIDO:version");
  return version;
}
function timestamp() {
  const now = /* @__PURE__ */ new Date();
  return now.toISOString().replace(/[-:TZ]/gu, "").slice(0, 14).replace(/^(\d{8})(\d{6})$/u, "$1.$2");
}
function readMemory() {
  return fs.readFileSync(MEMORY_PATH, "utf8");
}
function writeMemory(value) {
  fs.writeFileSync(MEMORY_PATH, String(value).replace(/\r?\n/gu, "\r\n"), "utf8");
}
function repositoryFrom(config) {
  const repository = String(config.upstreamRepository || "");
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/u.test(repository)) throw new UsageError("REPOSITORIO_ISSUE_INVALIDO");
  return repository;
}
function readConfig() {
  const pkg = JSON.parse(fs.readFileSync(path.join(ROOT_DIR, "package.json"), "utf8"));
  const local = path.join(ROOT_DIR, ".ia.rules", "upstream.json");
  return { ...pkg["agentsUpstream"] || {}, ...fs.existsSync(local) ? JSON.parse(fs.readFileSync(local, "utf8")) : {} };
}
function assertConstructor(options) {
  const role = options.role || readConfig().role;
  if (role !== "constructor" && role !== "dual") throw new Error("PAPEL_CONSTRUTOR_EXPLICITO_EXIGIDO");
}
...
async function github(config, pathname, method, options = {}) {
  return requestJson({ body: options.body, headers: { "User-Agent": "agents-governance-lifecycle", ...options.token ? { Authorization: `Bearer ${options.token}` } : {} }, maxBytes: config.maxBytes, method, retries: method === "GET" ? config.retries : 0, timeoutMs: config.timeoutMs, url: `${String(config.githubApi || GITHUB_API).replace(/\/$/u, "")}${pathname}` });
}
function projectResponse(response) {
  return { code: response.code, ok: response.ok, status: response.status };
}
function required(value, name) {
  if (!String(value || "").trim()) throw new UsageError(`PARAMETRO_NORMATIVO_AUSENTE:${name}`);
  return value;
}
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (value === "--role") result.role = argv[++i];
    else if (value === "--ft") result.ft = argv[++i];
    else if (value === "--version") result.version = argv[++i];
    else if (value === "--inbox-dir") result.inboxDir = argv[++i];
...
function help() {
  return "Uso: issue-lifecycle <sync-approved|start [--ft FT-NNN]|bind-release <versao>|complete-release <versao>> --role constructor [--authorize|--dry-run]\n";
}

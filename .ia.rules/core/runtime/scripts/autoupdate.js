...
async function withVirtualUpstream(policy, rootDir, operation) {
  ...
  fs.existsSync = function patchedExists(target) {
    return path.resolve(String(target)) === path.resolve(localPath) || originalExists.call(fs, target);
  };

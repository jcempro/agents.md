...
function locateTodoFiles(rootDir, options = {}) {
  ...
  return [...new Set(candidates)].filter((relativePath) => !path.isAbsolute(relativePath) && fs.existsSync(path.join(rootDir, relativePath))).sort((a, b) => toPosix(a).localeCompare(toPosix(b), "en"));
}
function parseTodoItems(content) {
  return String(content).split(/\r?\n/u).map((line, index) => ({ line, number: index + 1 })).filter((entry) => /^\s*[-*]\s+(?:\[[ xX-]\]\s+)?\S/u.test(entry.line)).map((entry) => ({
    line: entry.number,

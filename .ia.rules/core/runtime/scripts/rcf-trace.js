...
function validate(rootDir, options = {}) {
  const map = readMap(rootDir, options.mapPath);
  const byRcf = /* @__PURE__ */ new Map();
  const changedByCommit = /* @__PURE__ */ new Map();
  for (const entry of map.entries) {
    if (!entry.id || !entry.rcf || !entry.sentenceSha256 || !Array.isArray(entry.artifacts) || !["pending", "materialized", "synchronized"].includes(entry.state)) throw new Error(`RCF_ENTRADA_INVALIDA:${entry.id || "sem-id"}`);
    if (!byRcf.has(entry.rcf)) byRcf.set(entry.rcf, []);
    byRcf.get(entry.rcf).push(entry);
  }
  let material = 0;

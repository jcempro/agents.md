...
// Gerado de: src/.ia.rules/scenarios/release/scripts/release-workflow.ts; TypeScript 7.0.2 + esbuild 0.28.1; Node 24+.
...
function detectReleaseTrigger(args = []) {
  ...
  const match = commits.map((commit) => inspectCommitForRelease(commit)).filter(Boolean).pop();
  if (!match) {
...
function inspectCommitForRelease(commit) {
  const changed = runGit(["diff-tree", "--no-commit-id", "--name-status", "-r", commit], { optional: true }).stdout.trim().split(/\r?\n/u).filter(Boolean).map((line) => {

...
function resolvePrimaryBranch(options) {
  for (const branch of options.primary ? [options.primary] : ["main", "master"]) {
    if (run("git", ["ls-remote", "--exit-code", "--heads", options.remote, branch], { optional: true, timeout: 12e4 }).status === 0) return branch;
  }
...
function runTransient(command, args) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    result = run(command, args, { optional: true, timeout: 9e5 });
...
function run(command, args, options = {}) {
  const result = childProcess.spawnSync(command, args, { cwd: ROOT_DIR, encoding: "utf8", shell: false, timeout: options.timeout || 3e4 });

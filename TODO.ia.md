...
📜 **Reduzir estruturalmente consumo de tokens/contexto de solicitações, históricos e FTs, preservando integralmente semântica, rastreabilidade e retrocompatibilidade**
  - **Princípios**
    ...
    - É PROIBIDA redundância documental sem justificativa técnica plausível. Segregar, consolidar ou referenciar DEVE priorizar menor leitura/processamento sem perda semântica, rastreabilidade ou exigibilidade.
    - Crescimento contínuo e ilimitado de contexto/histórico DEVE ser prevenido arquiteturalmente.

  - **Histórico individual por solicitação**
    - Ao inicializar solicitação originada de `TO-DO`, Issue, Prompt ou equivalente, preservar seu conteúdo integral em **registro histórico individual**, identificado por **ID único e estável**.
    - O registro passa a ser referência canônica da solicitação original para consulta, auditoria, revalidação, comparação e rastreabilidade.
    - Arquivo único ou diretório identificado pelo ID PODE ser usado conforme convenções existentes e eficiência comprovada.
    - Anexos/evidências DEVEM permanecer no Git ou armazenamento canônico aplicável, vinculados inequivocamente ao histórico e facilmente localizáveis; PODEM residir junto ao diretório do ID.
    - O conteúdo original histórico é **imutável**.
    - Correções, esclarecimentos ou expansões posteriores do desenvolvedor DEVEM ser registrados como **adendos explicitamente identificados, ordenados e distinguíveis do original**, sem reescrevê-lo.

  - **`/TODO.ia.md`**
    - DEVE existir **somente na raiz** como instância canônica; homônimos somente PODEM existir como históricos sem valor operacional.
    - DEVE conter exclusivamente TO-DOs **não inicializadas ou em andamento**; estados terminais DEVEM ser removidos.
    - Após inicialização, substituir o conteúdo detalhado por checklist **ultrassucinto e inequivocamente correlacional**, contendo somente:
      - síntese mínima do que foi solicitado;
      - ID do histórico;
      - IDs/números das FTs relacionadas.

    - NÃO contextualizar, conceituar, justificar nem duplicar etapas/procedimentos presentes no histórico ou FT.
    - O resumo DEVE reduzir em **≥70%** os tokens do original quando materialmente possível sem prejudicar a correlação inequívoca; quando impossível, maximizar a redução e registrar objetivamente a causa.
    - Padronizar sintaxe do item raiz, subitens, histórico e referências a FTs.
    - Status iconizados e atualização de andamento aplicam-se **somente ao checklist resumido**, nunca ao histórico original.

  - **Issues**
    - O conteúdo original da Issue **NÃO DEVE ser reescrito**.
    - Criar histórico individual nos mesmos termos das demais solicitações.
    - Vincular Issue, ID histórico e FTs por comentário, metadado ou mecanismo mais eficiente suportado pelo estado real; PODE combinar mecanismos quando houver benefício concreto.
    - Correções/expansões posteriores DEVEM permanecer rastreáveis sem adulterar a solicitação original.
    - A relação Issue↔histórico↔FT DEVE ser bidirecional quando tecnicamente viável.

  - **Prompts de origem**
    - Solicitações originadas de **Prompt** DEVEM receber tratamento histórico e rastreabilidade equivalentes aos de Issue.
    - Como o Prompt original, por natureza, não é objeto operacionalmente editável nos mesmos termos de uma Issue, **NÃO se aplicam a ele regras específicas que dependam da edição/manutenção do artefato original**.
    - Seu conteúdo original DEVE, contudo, ser preservado integralmente no histórico individual.
    - Se o Prompt tiver como finalidade criar, corrigir, expandir ou complementar uma **TO-DO ou Issue**, ele DEVE ser tratado como origem de **adendo/correção/expansão da solicitação correspondente**, quando materialmente aplicável:
      - preservar Prompt e solicitação original;
      - vincular ambos pelo histórico/ID;
      - registrar explicitamente o que o Prompt acrescenta, corrige ou substitui;
      - NÃO reescrever retroativamente o histórico original.

    - Prompts independentes NÃO DEVEM ser artificialmente vinculados a TO-DOs/Issues apenas por similaridade temática.

  - **FTs**
    - Cada FT DEVE possuir arquivo individual; organização por diretório PODE ser usada para impedir leitura acidental de FTs não relacionadas.
    - A FT DEVE refletir semanticamente todos os requisitos aplicáveis da solicitação original, permitindo execução normal sem releitura integral do histórico.
    - FT mais compacta que o original somente é válida se:
      - nenhuma informação material for perdida ou enfraquecida;
      - houver referência inequívoca ao histórico;
      - o original for relido sempre que necessário para comparação, auditoria ou revalidação;
      - a estratégia resultar em menor custo global de tokens/processamento.

    - FTs não relacionadas à operação corrente NÃO DEVEM entrar no contexto por padrão.

  - **Arquivos canônicos de governança**
    - `/TODO.ia.md` e `/handoff.md` DEVEM residir obrigatoriamente na raiz.
    - `/handoff.md` DEVE ser o **único status global canônico**, resumido e ultrassucinto, de **todas as solicitações em andamento**, independentemente do tipo.
    - `continue.ia` DEVE possuir localização canônica **`/ia.rules/continue.ia`**, sem equivalente funcional concorrente.
    - Arquivos existentes em outros locais com papel equivalente DEVEM ser identificados, reconciliados e migrados aos paths canônicos também nos repositórios finais/consumidores.
    - Homônimos históricos NÃO PODEM ser interpretados como fonte operacional ou normativa.

  - **Retenção e limpeza**
    - Implementar ou reutilizar script específico de limpeza de históricos antigos com mecanismo próprio de execução automática.
    - A retenção DEVE:
      - impedir persistência eterna;
      - preservar solicitações ativas ou ainda referenciadas;
      - ser determinística, configurável e documentada;
      - derivar o prazo/critério do ciclo de vida real, evitando arbitrariedade.

    - A limpeza DEVE ser idempotente, auditável e incapaz de remover conteúdo ainda necessário.

  - **Migração e retrocompatibilidade**
    - Migrar estruturas legadas de modo resiliente, preservando IDs/referências existentes quando possível e impedindo perda documental.
    - A evolução DEVE oferecer:
      - migração integral automática/fail-safe; **ou**
      - convivência temporária entre formatos, com mapa explícito legado→novo e caminho para migração plena.

    - Nenhum consumidor/repositório final PODE ser quebrado silenciosamente.
    - Testar reexecução, migração interrompida/parcial, duplicidade, referência ausente e estados legados divergentes.

  - **Normatizar `fail-safe`/resiliência em `AGENTS.md`**
    - `fail-safe`, `resiliente` e equivalentes DEVEM significar **concluir efetivamente a operação solicitada apesar de falhas contornáveis**, e NÃO apenas falhar sem causar dano.
    - A execução DEVE tentar alternativas tecnicamente válidas — métodos, comandos ou estratégias distintas — até atingir o objetivo ou esgotar possibilidades legítimas.
    - Nenhum contorno PODE violar normas, segurança, contratos ou escopo.
    - Apenas impedimento materialmente incontornável justifica não conclusão; nesse caso, registrar tentativas, causa real e estado final.

  - **Documentação obrigatória de código-fonte**
    - Normatizar que **todo source produzido/editado por IA/subAgents DEVE ser documentalmente rastreável**.
    - É PROIBIDO produzir código-fonte sem comentários/documentação suficientes para terceiro compreender rapidamente localização, finalidade, responsabilidade e lógica.
    - Arquivos, funções, métodos, classes e unidades equivalentes DEVEM possuir cabeçalhos/documentação apropriados (`JSDoc` ou equivalente idiomático).
    - HTML, XML, CSS/(S)CSS, scripts e demais fontes DEVEM possuir comentários estruturais/lógicos equivalentes quando aplicáveis.
    - Comentários DEVEM explicar propósito, decisões e lógica não evidente; NÃO devem parafrasear trivialmente cada linha nem gerar prolixidade.
    - A regra NÃO se aplica a artefatos transpilados, minificados, gerados ou equivalentes destinados à distribuição e não mantidos como source.

  - **Otimização global**
    - Auditar arquivos carregados rotineiramente por IA/subAgents e identificar duplicações, crescimento acumulativo e contexto sem valor operacional.
    - Conteúdo redundante ou desnecessário DEVE ser removido, segregado ou substituído por referência quando isso reduzir custo total sem perda.
    - NÃO unificar arquivos por conveniência quando isso aumentar leitura irrelevante.
    - Preferir carregamento sob demanda por ID/FT/solicitação em vez de históricos monolíticos.

  - **Aceite**
    - Demonstrar, por medição reproduzível, redução substancial do contexto médio carregado em operações com TO-DOs, Issues, Prompts, FTs e históricos.
    - Validar redução ≥70% nos itens elegíveis de `/TODO.ia.md`.
    - Validar:
      - unicidade/estabilidade dos IDs;
      - imutabilidade do original + adendos rastreáveis;
      - anexos/evidências;
      - relações TO-DO/Issue/Prompt↔histórico↔FT;
      - Prompts que expandem/corrigem TO-DOs/Issues;
      - isolamento entre FTs;
      - paths canônicos;
      - ausência de equivalentes operacionais concorrentes;
      - limpeza automática/idempotente e proteção de referências ativas;
      - migração legado→novo, reexecução e recuperação após falha.

    - Confirmar que nenhuma informação material, feature, contrato ou capacidade existente sofreu regressão.
